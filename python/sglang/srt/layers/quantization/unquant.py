...
class UnquantizedLinearMethod(LinearMethodBase):
    ...
    def apply(
        ...
        if _is_npu and is_mla_preprocess_enabled() and "fused_qkv_a_proj_with_mqa" in layer.prefix:
            out1 = torch.matmul(x, layer.weightNz1)
            out2 = torch.matmul(x, layer.weightNz2)
            return torch.cat((out1, out2), dim=-1)            
        elif _is_npu and is_mla_preprocess_enabled() and "q_b_proj" in layer.prefix:
            return torch.matmul(x, layer.weightNz)
        else:
